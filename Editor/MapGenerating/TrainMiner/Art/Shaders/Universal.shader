...
Shader "Flexus/Universal v6.1.0"
{
    Properties
    {
        ...
        [Space(10)][KeywordEnum(Disabled, Fresnel, World X, World Y, World Z, Object X, Object Y, Object Z, Texture Mask, Atlas Y)] USE_GRADIENT ("Градієнт кольору", Int) = 0
        ...
        [Space(10)][KeywordEnum(Final Render, Normal, Vertex normals, Normal map, UV, NdotV, NdotL, NdotH, View vector, Reflection vector, Half vector, Fresnel, Specular, Cubemap, Lambert, Shadow, Light, Lightmap, Diffuse, Albedo, Roughness, Metallic, Occlusion, Curvature, World Position, Model Position)] DEBUG_MODE ("Оверлеі для дебагу", Int) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature USE_GRADIENT_DISABLED USE_GRADIENT_FRESNEL USE_GRADIENT_WORLD_X USE_GRADIENT_WORLD_Y USE_GRADIENT_WORLD_Z USE_GRADIENT_OBJECT_X USE_GRADIENT_OBJECT_Y USE_GRADIENT_OBJECT_Z USE_GRADIENT_TEXTURE_MASK USE_GRADIENT_ATLAS_Y
            #pragma shader_feature LIGHT_MODE_LAMBERT LIGHT_MODE_UNLIT LIGHT_MODE_SOFT_LAMBERT LIGHT_MODE_BAKED_ONLY
            #pragma shader_feature SPECULAR_MODE_SMOOTH SPECULAR_MODE_SHARP
			...
			half4 frag (universal_v2f i) : SV_Target
            {
                ...
                half4 color;
                    ...
                    half gradient_y = saturate((i.ws_pos.y - _GradientSettings.x) / (_GradientSettings.y - _GradientSettings.x));
                    half4 gradient = tex2D(_MainTex, half2(i.uv.x, gradient_y));
                    data.tint = _Color;
                    data.tex.rgb = gradient.rgb * (2. - gradient.a);
                    data.tex.a = 1.;
                    data.albedo = data.tint.rgb * data.tex.rgb;
                    color.a = data.tint.a;
                #else
                    ...
                    data.albedo = lerp(_Color.rgb, _Color2.rgb, data.tex.r);
                    color.a = lerp(_Color.a, _Color2.a, data.tex.r);
                #endif
                UNIVERSAL_MOD_ALBEDO_INJECTION
                #if ALBEDO_ALPHA_IS_AO
                data.occlusion = data.tex.a;
                #endif
                ...
                REQUIRED_UNIVERSAL_HLSL_VERSION_610
                ...
                //float f = data.NdotV * data.lambert_raw;
                //color.rgb += saturate(abs((data.NdotH + _Time.z * 0.5) % 4 - 2) - 1.8) * _Color;//lerp(_Color.rgb, _Color2.rgb, (data.NdotH - data.fresnel));
