//Supports Universal 6.1.0
REQUIRED_FLEXUS_CORE_HLSL_VERSION_131
#define REQUIRED_UNIVERSAL_HLSL_VERSION_610

//TODO ATLAS Y OPTIMIZE INTERPOLATORS, MOVE TEX2D TO VERTEX
...
#if defined(USE_TEXTURE) || defined(ALBEDO_ALPHA_IS_AO) || defined(USE_NORMAL_MAP) || defined(NOR_BLUE_IS_NONMETALLIC) || defined(NOR_ALPHA_IS_ROUGHNESS) || defined(TEST_TEXTURES) || USE_EMISSION_TEX || USE_GRADIENT_ATLAS_Y
	#define UV_NEEDED 1
#endif

#if defined(USE_GRADIENT_WORLD_X) || defined(USE_GRADIENT_WORLD_Y) || defined(USE_GRADIENT_WORLD_Z) || defined(USE_GRADIENT_OBJECT_X) || defined(USE_GRADIENT_OBJECT_Y) || defined(USE_GRADIENT_OBJECT_Z) || defined(USE_GRADIENT_ATLAS_Y)
...
inline half4 UniversalGlowStylized(half fresnel, half threshold, half4 color) {
	return (fresnel > threshold) * color;
}
...
inline half UniversalSpecularStylized(float d, half roughness) {
	half oneMinusRsqr = 1. - roughness * roughness;
	return (d * d > oneMinusRsqr) * oneMinusRsqr;
}
...
#ifdef USE_GLOW
	#if SPECULAR_MODE_SMOOTH
		#define UNIVERSAL_ADD_GLOW color += UniversalGlow(data.fresnel, GET(_GlowPower), GET(_GlowColor));
	#endif
	#if SPECULAR_MODE_SHARP
		#define UNIVERSAL_ADD_GLOW color += UniversalGlowStylized(data.fresnel, GET(_GlowPower), GET(_GlowColor));
	#endif
#else
	#define UNIVERSAL_ADD_GLOW
#endif

#if !SPECULAR_MODE_SMOOTH && !SPECULAR_MODE_SHARP
 	#define CALCULATE_SPECULAR data.specular = 0.;
#endif
#if SPECULAR_MODE_SMOOTH
	#define CALCULATE_SPECULAR data.specular = UniversalSpecular(data.NdotH, data.roughness) * __specular_mult * GET(_SpecularMultiplier);
#endif
#if SPECULAR_MODE_SHARP
	#define CALCULATE_SPECULAR data.specular = UniversalSpecularStylized(data.NdotH, data.roughness) * __specular_mult * GET(_SpecularMultiplier);
#endif
...
#endif
#if USE_GRADIENT_TEXTURE_MASK
#define UNIVERSAL_APPLY_GRADIENT_TINT data.tint = 1.;
#endif
#if USE_GRADIENT_ATLAS_Y
#define UNIVERSAL_APPLY_GRADIENT_TINT data.tint = 1.;
#endif
